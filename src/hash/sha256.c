...
/*   Updated: 2018/10/08 12:16:43 by acazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hash		g_hash_sha256 = {"SHA256",
	(t_hash_init)sha256_init,
	(t_hash_update)sha256_update,
	(t_hash_final)sha256_final,
	32, 64, sizeof(t_sha256_ctx)};
...
static void	sha256_loop(int i, uint32_t *tmp, uint32_t *w)
{
	uint32_t	tmp1;
	uint32_t	tmp2;
	uint32_t	tmp3;
	...
	tmp3 = tmp[7] + tmp1 + tmp2 + g_sha256_k[i] + w[i];
...
static void	sha256_chunk(t_sha256_ctx *ctx)
{
	uint32_t	tmp[8];
	uint32_t	w[64];
	int			i;
