/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   aria_round.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: acazuc <acazuc@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/04 12:38:24 by acazuc            #+#    #+#             */
/*   Updated: 2019/08/04 16:20:08 by acazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cipher/aria.h"
#include "ft_ssl.h"

static uint8_t aria_sb1[256] = {
	};

static uint8_t aria_sb2[256] = {
	};

static uint8_t aria_sb3[256] = {
	};

static uint8_t aria_sb4[256] = {
	};

void	aria_sl1(uint8_t *dst, uint8_t *src)
{
	dst[0x0] = aria_sb1[src[0x0]];
	dst[0x1] = aria_sb2[src[0x1]];
	dst[0x2] = aria_sb3[src[0x2]];
	dst[0x3] = aria_sb4[src[0x3]];
	dst[0x4] = aria_sb1[src[0x4]];
	dst[0x5] = aria_sb2[src[0x5]];
	dst[0x6] = aria_sb3[src[0x6]];
	dst[0x7] = aria_sb4[src[0x7]];
	dst[0x8] = aria_sb1[src[0x8]];
	dst[0x9] = aria_sb2[src[0x9]];
	dst[0xa] = aria_sb3[src[0xa]];
	dst[0xb] = aria_sb4[src[0xb]];
	dst[0xc] = aria_sb1[src[0xc]];
	dst[0xd] = aria_sb2[src[0xd]];
	dst[0xe] = aria_sb3[src[0xe]];
	dst[0xf] = aria_sb4[src[0xf]];
}

void	aria_sl2(uint8_t *dst, uint8_t *src)
{
	dst[0x0] = aria_sb3[src[0x0]];
	dst[0x1] = aria_sb4[src[0x1]];
	dst[0x2] = aria_sb1[src[0x2]];
	dst[0x3] = aria_sb2[src[0x3]];
	dst[0x4] = aria_sb3[src[0x4]];
	dst[0x5] = aria_sb4[src[0x5]];
	dst[0x6] = aria_sb1[src[0x6]];
	dst[0x7] = aria_sb2[src[0x7]];
	dst[0x8] = aria_sb3[src[0x8]];
	dst[0x9] = aria_sb4[src[0x9]];
	dst[0xa] = aria_sb1[src[0xa]];
	dst[0xb] = aria_sb2[src[0xb]];
	dst[0xc] = aria_sb3[src[0xc]];
	dst[0xd] = aria_sb4[src[0xd]];
	dst[0xe] = aria_sb1[src[0xe]];
	dst[0xf] = aria_sb2[src[0xf]];
}

void	aria_a(uint8_t *dst, uint8_t *src)
{
	dst[0x0] = src[0x3] ^ src[0x4] ^ src[0x6] ^ src[0x8] ^ src[0x9] ^ src[0xd] ^ src[0xe];
	dst[0x1] = src[0x2] ^ src[0x5] ^ src[0x7] ^ src[0x8] ^ src[0x9] ^ src[0xc] ^ src[0xf];
	dst[0x2] = src[0x1] ^ src[0x4] ^ src[0x6] ^ src[0xa] ^ src[0xb] ^ src[0xc] ^ src[0xf];
	dst[0x3] = src[0x0] ^ src[0x5] ^ src[0x7] ^ src[0xa] ^ src[0xb] ^ src[0xd] ^ src[0xe];
	dst[0x4] = src[0x0] ^ src[0x2] ^ src[0x5] ^ src[0x8] ^ src[0xb] ^ src[0xe] ^ src[0xf];
	dst[0x5] = src[0x1] ^ src[0x3] ^ src[0x4] ^ src[0x9] ^ src[0xa] ^ src[0xe] ^ src[0xf];
	dst[0x6] = src[0x0] ^ src[0x2] ^ src[0x7] ^ src[0x9] ^ src[0xa] ^ src[0xc] ^ src[0xd];
	dst[0x7] = src[0x1] ^ src[0x3] ^ src[0x6] ^ src[0x8] ^ src[0xb] ^ src[0xc] ^ src[0xd];
	dst[0x8] = src[0x0] ^ src[0x1] ^ src[0x4] ^ src[0x7] ^ src[0xa] ^ src[0xd] ^ src[0xf];
	dst[0x9] = src[0x0] ^ src[0x1] ^ src[0x5] ^ src[0x6] ^ src[0xb] ^ src[0xc] ^ src[0xe];
	dst[0xa] = src[0x2] ^ src[0x3] ^ src[0x5] ^ src[0x6] ^ src[0x8] ^ src[0xd] ^ src[0xf];
	dst[0xb] = src[0x2] ^ src[0x3] ^ src[0x4] ^ src[0x7] ^ src[0x9] ^ src[0xc] ^ src[0xe];
	dst[0xc] = src[0x1] ^ src[0x2] ^ src[0x6] ^ src[0x7] ^ src[0x9] ^ src[0xb] ^ src[0xc];
	dst[0xd] = src[0x0] ^ src[0x3] ^ src[0x6] ^ src[0x7] ^ src[0x8] ^ src[0xa] ^ src[0xd];
	dst[0xe] = src[0x0] ^ src[0x3] ^ src[0x4] ^ src[0x5] ^ src[0x9] ^ src[0xb] ^ src[0xe];
	dst[0xf] = src[0x1] ^ src[0x2] ^ src[0x4] ^ src[0x5] ^ src[0x8] ^ src[0xa] ^ src[0xf];
}

void	aria_fo(uint8_t *dst, uint8_t *d, uint8_t *rk)
{
	uint8_t	tmp[16];

	ft_memxor(tmp, d, rk, 16);
	aria_sl1(tmp, tmp);
	aria_a(dst, tmp);
}

void	aria_fe(uint8_t *dst, uint8_t *d, uint8_t *rk)
{
	uint8_t	tmp[16];

	ft_memxor(tmp, d, rk, 16);
	aria_sl2(tmp, tmp);
	aria_a(dst, tmp);
}
